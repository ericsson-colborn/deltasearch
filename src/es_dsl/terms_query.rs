        ...
        for json_val in values {
            let term = match field_type {
                ...
                FieldType::Boolean => {
                    let text = json_val.as_str().ok_or_else(|| {
                        SearchDbError::Schema(format!(
                            "terms query on field \"{field_name}\" expected boolean string, got {json_val}"
                        ))
                    })?;
                    let b = match text {
                        "true" => true,
                        ...
                        _ => {
                            return Err(SearchDbError::Schema(format!(
                                "terms query on field \"{field_name}\" expected \"true\" or \"false\", got \"{text}\""
                            )))
                        }
                    };
                ...
                FieldType::Ip => {
                    let ip_str = json_val.as_str().ok_or_else(|| {
                        SearchDbError::Schema(format!(
                            "terms query on field \"{field_name}\" expected IP string, got {json_val}"
                        ))
                    })?;
                    let ip: std::net::Ipv6Addr = match ip_str.parse::<std::net::IpAddr>() {
                        ...
                        Err(e) => {
                            return Err(SearchDbError::Schema(format!(
                                "terms query on field \"{field_name}\" invalid IP \"{ip_str}\": {e}"
                            )))
                        }
                    };

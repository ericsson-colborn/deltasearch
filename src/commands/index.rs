...
fn rebuild_index(storage: &Storage, name: &str, new_schema: &Schema) -> Result<()> {
    ...
    eprintln!(
        "[dewey] Rebuilt index '{name}' with {} existing document(s)",
        existing_docs.len()
    );
    Ok(())
}
...
fn read_ndjson(file: Option<&str>) -> Result<Vec<serde_json::Value>> {
    ...
    for line in reader.lines() {
        ...
        match serde_json::from_str(trimmed) {
            Ok(v) => docs.push(v),
            Err(e) => eprintln!("[dewey] Skipping invalid JSON: {e}"),
        }
    }
    Ok(docs)
}
...
pub fn run(storage: &Storage, name: &str, file: Option<&str>) -> Result<()> {
    ...
    if new_docs.is_empty() {
        eprintln!("[dewey] No documents to index");
        return Ok(());
    }
    ...
    if config.inferred {
        ...
        if !new_fields.is_empty() {
            eprintln!(
                "[dewey] Schema evolution: discovered {} new field(s): {}",
                new_fields.len(),
                new_fields.join(", ")
            );
    ...
    eprintln!("[dewey] Indexed {count} document(s) into '{name}'");
    Ok(())
}
